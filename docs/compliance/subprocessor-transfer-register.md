...
1. Complete **TRA / data protection test** for each restricted transfer actually used **except** transfers to **Vercel Inc.** covered by its **ACTIVE UK Extension** (Neon, Resend, and any enabled Twilio/Sentry/Slack/Google/Stripe/OpenAI path). Vendor SCCs/IDTA alone do **not** complete a required test where adequacy does not apply.
