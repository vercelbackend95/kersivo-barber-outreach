...
7. Obtain/confirm **signed current vendor DPAs** and record the **exact** UK safeguard for remaining providers (enabled Twilio/Stripe; controller-side GA4/Ads as applicable — **not** Google Sign-In TRA).
...
4. Controller-side TRA coverage for **Stripe**, **GA4**, and **Google Ads** (Google Sign-In / OAuth: **no Schedule-2 TRA** on audited facts — see exclusion note).
