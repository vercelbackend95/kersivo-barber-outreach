...
7. Obtain/confirm **signed current vendor DPAs** and record the **exact** UK safeguard for remaining providers (enabled Twilio; controller-side GA4/Ads as applicable — **not** Google Sign-In TRA). **Stripe:** SSA/DPA with SPEL for UK account **CONFIRMED** via official Stripe terms; primary UK Extension for Stripe, LLC — periodically recheck.
...
4. Controller-side TRA coverage for **GA4** and **Google Ads** (Google Sign-In / OAuth: **no Schedule-2 TRA** on audited facts). **Stripe SaaS:** TRA not required while UK Extension covers Stripe, LLC — periodic recheck only.
