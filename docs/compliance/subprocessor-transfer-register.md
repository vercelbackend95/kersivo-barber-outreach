...
| 8 | **Google Analytics 4** | Marketing-site analytics — **ACTIVE** / consent-based | Pseudonymous usage analytics as described in Privacy | **Google = processor** for Analytics-service data under current verified configuration (**Google products & services** data sharing = **OFF**). If that data-sharing setting were enabled in future, Google may become an independent controller for data shared under the relevant controller-controller terms — do not enable without re-review. KERSIVO remains controller for deciding to use GA4 on the marketing site | Analytics provider (processor path for current Analytics-service data) | Conditional on `PUBLIC_GA4_MEASUREMENT_ID` + analytics consent + layout `enableAnalytics` (**off** on live tenant book/shop — hard-off **CLOSED**) | **VERIFY** (Google; CSP includes `region1.google-analytics.com` etc.) | **VERIFY** — exact UK safeguard / restricted-transfer conclusion **OPEN** | Google Analytics DPA / UK transfer safeguards — **VERIFY** (do **not** invent completed TRA, importer, or adequacy conclusion here) | Privacy; Cookie Policy; DPA Schedule 2 exclusions (marketing); Compliance Diary Phase 2B | **TRA STATUS: NOT YET COMPLETED — OPEN / VERIFY** | Consent Mode; tenant analytics disabled; products & services sharing OFF | 2026-09-24 | **Next:** GA4 contract / transfer / account data-control finalisation |
...
| Google Analytics 4 | **B** | KERSIVO marketing-controller use of GA4; Google = Analytics-service **processor** while products & services sharing OFF; tenant tags off; DPA exclusion; transfer TRA still **OPEN / VERIFY** |
...
7. Obtain/confirm **signed current vendor DPAs** and record the **exact** UK safeguard for remaining providers (enabled Twilio; GA4 transfer finalisation — **not** Google Sign-In TRA; Google Ads dormant — reopen if re-enabled). **Stripe:** SSA/DPA with SPEL for UK account **CONFIRMED** via official Stripe terms; primary UK Extension for Stripe, LLC — periodically recheck.
...
4. Controller-side GA4 contractual / international-transfer finalisation (**OPEN / VERIFY**). Google Ads: **INACTIVE / DORMANT** — reopen TRA only if re-enabled (Google Sign-In / OAuth: **no Schedule-2 TRA** on audited facts). **Stripe SaaS:** TRA not required while UK Extension covers Stripe, LLC — periodic recheck only.
