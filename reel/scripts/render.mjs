/**
 * Renders ZeroCommissionReel to MP4. Requires FFmpeg on PATH.
...
const outFile = join(outDir, 'zero-commission-reel.mp4');
const tempFile = join(outDir, 'zero-commission-reel-2x.mp4');

function checkFfmpeg() {
  const result = spawnSync('ffmpeg', ['-version'], { encoding: 'utf8', shell: true });
  if (result.status !== 0) {
    console.error('[reel:render] FFmpeg not found on PATH. Install from https://ffmpeg.org/');
    process.exit(1);
  }
  console.log('[reel:render] FFmpeg OK');
}
...
async function main() {
  ...
  console.log('[reel:render] Rendering 2x supersample:', tempFile);
  ...
  console.log('[reel:render] Downscaling to 1080x1920:', outFile);
  ...
  console.log('[reel:render] Done:', outFile);
}
