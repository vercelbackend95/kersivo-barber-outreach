...
import { processDueAppointmentReminders } from '../../../lib/sms/reminders';
...
async function handle(request: Request): Promise<Response> {
  ...
  try {
    const summary = await processDueAppointmentReminders(new Date());
    ...
    return new Response(
      JSON.stringify({
        ok: false,
        error: error instanceof Error ? error.message : 'SMS reminder cron failed',
      }),
