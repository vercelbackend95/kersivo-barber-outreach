...
vi.mock('@/lib/setup/shopPublicBookingGate', () => ({
  shopAcceptsPublicBookings: vi.fn(async (shopId: string) => shopId !== DEMO_SHOP_ID),
}));
...
describe('GET /api/public/bookings/[shopId]/availability', () => {
  ...
  it('returns 400 when shop id is missing', async () => {
    const res = await GET(makeContext(undefined, { serviceId: serviceA, barberId: barberA, date: '2026-07-20' }) as never);
