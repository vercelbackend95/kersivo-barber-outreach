...
describe('Product settings demo lock', () => {
  it('blocks startEdit with the shared demo lock before opening the editor', () => {
    const source = readFileSync(resolve(process.cwd(), 'src/components/admin/ShopAdminPanel.tsx'), 'utf8');
    ...
    expect(source).toContain('Product settings — sample data is read-only');
