...
const HISTORY_ID_PREFIX = 'connor-history-bk-';
const LEGACY_SEED_NAME = 'Harper Quinn';
const LEGACY_SEED_EMAIL = 'timeline.mock.afternoon21@example.com';
const CLIENT_FULL_NAME = 'Connor Walsh';
const CLIENT_EMAIL = 'connor.walsh@gmail.com';
const CLIENT_PHONE = '07841 293756';
const TARGET_START_TIME = '16:45';
const HISTORY_VISIT_COUNT = 20;
const DAYS_BETWEEN_VISITS = 21;

const CLIENT_TAGS = ['VIP', 'Regular', 'Haircut + Beard', 'Prefers Leo', 'Beard care'];
...
const VISIT_START_TIMES = ['14:00', '14:15', '16:45', '17:00'] as const;

/** Service names weighted toward Haircut + Beard as favourite. */
...
/** ~60% Leo, rest split between Mason and Theo. */
...
function historyManageToken(index: number): string {
  return `connor-history-manage-${String(index).padStart(2, '0')}`;
}
...
async function findTargetBooking(date: string) {
  ...
  const dayBookings = await prisma.booking.findMany({
    where: {
      ...
      OR: [
        { fullName: { in: [LEGACY_SEED_NAME, CLIENT_FULL_NAME] } },
  ...
  const atTargetTime = dayBookings.find((row) => formatLondonTime(row.startAt) === TARGET_START_TIME);
  if (atTargetTime) return atTargetTime;
  ...
  throw new Error(
    `Could not find ${LEGACY_SEED_NAME}/${CLIENT_FULL_NAME} booking at ${TARGET_START_TIME} on ${date}. Run seed:timeline-afternoon first.`,
  );
}
...
async function loadServiceByName(name: string, services: Service[]): Promise<Service> {
  ...
  const haircutBeard = services.find((service) =>
    service.name.toLowerCase().includes('haircut') && service.name.toLowerCase().includes('beard'),
  );
  if (haircutBeard) return haircutBeard;
...
async function main() {
  ...
  console.info(`[enrich-connor-walsh] Target date (Europe/London): ${date}`);

  const settings = await prisma.shopSettings.findFirstOrThrow();
  const targetBooking = await findTargetBooking(date);
  console.info(
    `[enrich-connor-walsh] Found booking ${targetBooking.id} at ${formatLondonTime(targetBooking.startAt)} · ${targetBooking.barber.name}`,
  );
  ...
  if (deletedHistory.count > 0) {
    console.info(`[enrich-connor-walsh] Removed ${deletedHistory.count} previous history booking(s).`);
  }
  ...
  if (oldEmail !== CLIENT_EMAIL.toLowerCase()) {
    ...
    if (orphan && orphan.id !== client.id) {
      ...
      if (linkedCount === 0) {
        ...
        console.info(`[enrich-connor-walsh] Removed orphan client record for ${targetBooking.email}.`);
      }
    }
  }
  ...
  console.info(`[enrich-connor-walsh] Client: ${CLIENT_FULL_NAME} <${CLIENT_EMAIL}>`);
  console.info(`[enrich-connor-walsh] Tags: ${CLIENT_TAGS.join(', ')}`);
  console.info('[enrich-connor-walsh] Stats:');
  ...
  console.info('[enrich-connor-walsh] Recent history:');
...
main()
  ...
  .catch(async (error) => {
    console.error('[enrich-connor-walsh] Failed:', error instanceof Error ? error.message : error);
