...
async function main() {
  ...
  console.info(`[timeline-today] Target date (Europe/London): ${date}`);
  console.info(`[timeline-today] Barbers: ${TARGET_BARBERS.join(', ')}`);

  const deleted = await prisma.booking.deleteMany({
    where: { id: { startsWith: BOOKING_ID_PREFIX } }
  });
  if (deleted.count > 0) {
    console.info(`[timeline-today] Removed ${deleted.count} previous timeline mock booking(s).`);
  }
  ...
  if (picked.length < TARGET_BOOKING_COUNT) {
    console.warn(
      `[timeline-today] Only ${picked.length}/${TARGET_BOOKING_COUNT} non-overlapping slots available; inserting what fits.`
    );
  }
  ...
  console.info(`[timeline-today] Inserted ${picked.length} booking(s) for ${date}.`);
  console.info('[timeline-today] Distribution per barber:', perBarber);
  console.info(
    '[timeline-today] Sample:',
...
main()
  ...
  .catch(async (error) => {
    console.error('[timeline-today] Failed:', error instanceof Error ? error.message : error);
